...
class GraphObjectPagingLoader<T extends GraphObject> extends Loader<SimpleGraphObjectCursor<T>>
{
    ...
    private CacheableRequestBatch putRequestIntoBatch(final Request request, final boolean b) {
        ...
        boolean forceRoundTrip = false;
        if (!b) {
            forceRoundTrip = true;
        }
        cacheableRequestBatch.setForceRoundTrip(forceRoundTrip);
    ...
    private void startLoading(final Request currentRequest, final boolean skipRoundtripIfCached, final long n) {
        ...
        final Runnable runnable = new Runnable() {
            private final /* synthetic */ RequestBatch val$batch = GraphObjectPagingLoader.this.putRequestIntoBatch(currentRequest, skipRoundtripIfCached);
    ...
    {
        void onError(FacebookException p0, GraphObjectPagingLoader<?> p1);
    }
