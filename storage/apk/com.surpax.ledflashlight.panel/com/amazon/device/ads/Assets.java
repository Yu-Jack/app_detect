// 
// Decompiled by Procyon v0.5.29
// 

package com.amazon.device.ads;

import java.io.File;
import java.io.FileOutputStream;
import java.io.IOException;
import java.io.OutputStream;
import java.io.BufferedOutputStream;

class Assets
{
    public static final String CLOSE = "amazon_ads_close.png";
    public static final String CLOSE_NORMAL = "amazon_ads_close_button_normal.png";
    public static final String CLOSE_PRESSED = "amazon_ads_close_button_pressed.png";
    private static Assets INSTANCE;
    public static final String INTERSITIAL_CLOSE_NORMAL = "amazon_ads_interstitial_close_normal.png";
    public static final String INTERSITIAL_CLOSE_PRESSED = "amazon_ads_interstitial_close_pressed.png";
    public static final String LEFT_ARROW = "amazon_ads_leftarrow.png";
    private static final String LOG_TAG = "Assets";
    public static final String OPEN_EXTERNAL_BROWSER = "amazon_ads_open_external_browser.png";
    public static final String REFRESH = "amazon_ads_refresh.png";
    public static final String RIGHT_ARROW = "amazon_ads_rightarrow.png";
    private static final int VERSION = 4;
    private static final String VERSION_SETTINGS_KEY = "mraid-assets-version";
    private String baseFilePath;
    
    static {
        Assets.INSTANCE = new Assets();
    }
    
    protected Assets() {
        this.baseFilePath = this.getBaseFilePath();
    }
    
    public static final Assets getInstance() {
        return Assets.INSTANCE;
    }
    
    protected static void newInstance() {
        Assets.INSTANCE = new Assets();
    }
    
    protected static void setInstance(final Assets instance) {
        Assets.INSTANCE = instance;
    }
    
    private void writeAssetToDisk(final Assets$CircuitBreaker assets$CircuitBreaker, final byte[] array, final String s) {
        if (assets$CircuitBreaker.isBroken()) {
            return;
        }
        BufferedOutputStream bufferedOutputStream;
        try {
            bufferedOutputStream = new BufferedOutputStream(this.createOutputStream(s));
            final Assets assets = this;
            final BufferedOutputStream bufferedOutputStream2 = bufferedOutputStream;
            final byte[] array2 = array;
            assets.writeBytesToOutputStream(bufferedOutputStream2, array2);
            try {
                final BufferedOutputStream bufferedOutputStream3 = bufferedOutputStream;
                bufferedOutputStream3.flush();
                final BufferedOutputStream bufferedOutputStream4 = bufferedOutputStream;
                bufferedOutputStream4.close();
                return;
            }
            catch (IOException ex) {
                return;
            }
        }
        catch (IOException ex2) {
            Log.e("Assets", "Could not write an asset to disk: %s", s);
            Assets$CircuitBreaker.access$100(assets$CircuitBreaker);
            return;
        }
        try {
            final Assets assets = this;
            final BufferedOutputStream bufferedOutputStream2 = bufferedOutputStream;
            final byte[] array2 = array;
            assets.writeBytesToOutputStream(bufferedOutputStream2, array2);
            final BufferedOutputStream bufferedOutputStream3 = bufferedOutputStream;
            bufferedOutputStream3.flush();
            final BufferedOutputStream bufferedOutputStream4 = bufferedOutputStream;
            bufferedOutputStream4.close();
        }
        catch (IOException ex3) {
            Log.w("Assets", "Caught IOException while writing asset to disk: %s", s);
            Assets$CircuitBreaker.access$100(assets$CircuitBreaker);
            try {
                bufferedOutputStream.flush();
                bufferedOutputStream.close();
            }
            catch (IOException ex4) {}
        }
        finally {
            try {
                bufferedOutputStream.flush();
                bufferedOutputStream.close();
            }
            catch (IOException ex5) {}
        }
    }
    
    private boolean writeAssetsToDisk() {
        final Assets$CircuitBreaker assets$CircuitBreaker = new Assets$CircuitBreaker(this);
        this.writeBase64EncodedAssetToDisk(assets$CircuitBreaker, "", this.getFilePath("amazon_ads_close_button_normal.png"));
        this.writeBase64EncodedAssetToDisk(assets$CircuitBreaker, "", this.getFilePath("amazon_ads_close_button_pressed.png"));
        this.writeBase64EncodedAssetToDisk(assets$CircuitBreaker, "", this.getFilePath("amazon_ads_close.png"));
        this.writeBase64EncodedAssetToDisk(assets$CircuitBreaker, "", this.getFilePath("amazon_ads_leftarrow.png"));
        this.writeBase64EncodedAssetToDisk(assets$CircuitBreaker, "", this.getFilePath("amazon_ads_refresh.png"));
        this.writeBase64EncodedAssetToDisk(assets$CircuitBreaker, "", this.getFilePath("amazon_ads_rightarrow.png"));
        this.writeBase64EncodedAssetToDisk(assets$CircuitBreaker, "", this.getFilePath("amazon_ads_interstitial_close_normal.png"));
        this.writeBase64EncodedAssetToDisk(assets$CircuitBreaker, "", this.getFilePath("amazon_ads_interstitial_close_pressed.png"));
        this.writeBase64EncodedAssetToDisk(assets$CircuitBreaker, "", this.getFilePath("amazon_ads_open_external_browser.png"));
        return !assets$CircuitBreaker.isBroken();
    }
    
    private void writeBase64EncodedAssetToDisk(final Assets$CircuitBreaker assets$CircuitBreaker, final String s, final String s2) {
        this.writeAssetToDisk(assets$CircuitBreaker, Base64.decode(s), s2);
    }
    
    protected OutputStream createOutputStream(final String name) {
        return new FileOutputStream(name);
    }
    
    public boolean ensureAssetsCreated() {
        while (true) {
            while (true) {
                synchronized (this) {
                    Label_0036: {
                        if (this.baseFilePath != null) {
                            break Label_0036;
                        }
                        this.baseFilePath = this.getBaseFilePath();
                        final String baseFilePath = this.baseFilePath;
                        final boolean b = false;
                        if (baseFilePath != null) {
                            break Label_0036;
                        }
                        return b;
                    }
                    final long nanoTime = System.nanoTime();
                    final Settings instance = Settings.getInstance();
                    final int int1 = instance.getInt("mraid-assets-version", 0);
                    boolean b2 = false;
                    if (int1 < 4) {
                        Log.d("Assets", "Writing assets to disk: version %d", 4);
                        if (!this.writeAssetsToDisk()) {
                            Metrics.getInstance().getMetricsCollector().incrementMetric(Metrics$MetricType.ASSETS_FAILED);
                            return false;
                        }
                        instance.putInt("mraid-assets-version", 4);
                        b2 = true;
                    }
                    final long convertToMillisecondsFromNanoseconds = Utils.convertToMillisecondsFromNanoseconds(System.nanoTime() - nanoTime);
                    final MetricsCollector metricsCollector = Metrics.getInstance().getMetricsCollector();
                    if (b2) {
                        final Metrics$MetricType metrics$MetricType = Metrics$MetricType.ASSETS_CREATED_LATENCY;
                        metricsCollector.publishMetricInMilliseconds(metrics$MetricType, convertToMillisecondsFromNanoseconds);
                        return true;
                    }
                }
                final Metrics$MetricType metrics$MetricType = Metrics$MetricType.ASSETS_ENSURED_LATENCY;
                continue;
            }
        }
    }
    
    protected String getBaseFilePath() {
        if (InternalAdRegistration.getInstance() != null && InternalAdRegistration.getInstance().getFilesDir() != null) {
            return InternalAdRegistration.getInstance().getFilesDir().getAbsolutePath() + File.separator;
        }
        return null;
    }
    
    public String getFilePath(final String str) {
        return this.baseFilePath + str;
    }
    
    protected void writeBytesToOutputStream(final OutputStream outputStream, final byte[] b) {
        outputStream.write(b);
    }
}
